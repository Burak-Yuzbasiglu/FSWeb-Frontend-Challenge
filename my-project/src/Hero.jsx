        ...
        <div className={`flex justify-center items-center py-16 ${isDarkMode ? 'bg-white text-black' : 'bg-[#252128] text-white'}`}>
            <div className="flex justify-between items-center max-w-8xl mx-auto px-8">
                <div className="flex flex-col justify-center items-start max-w-xl">
                    ...
                    <h1 className={`text-7xl font-bold text-[#1F2937] leading-none mb-0 ${isDarkMode ? 'text-[#1F2937]' : 'text-[#AEBCCF]'}`}>
                        {!isTurkish ? 'Yaratıcı Düşünen' : 'Creative thinker'}
                    </h1>
                    <h1 className={`text-7xl font-bold text-[#1F2937] leading-none mb-8 ${isDarkMode ? 'text-[#1F2937]' : 'text-[#AEBCCF]'}`}>
                        {!isTurkish ? 'Minimalizm Aşığı' : 'Minimalism lover'}
                    </h1>
                    <p className={`text-lg font-normal text-[#6B7280] mb-8 ${isDarkMode ? 'text-[#6B7280]' : 'text-[#FFFFFF]'}`}>
                        {!isTurkish
                            ? 'Merhaba, ben Almila. Ben bir full-stack geliştiricisiyim. Harika kullanıcı deneyimlerine sahip sağlam ve ölçeklenebilir frontend ürünleri oluşturacak bir geliştirici arıyorsanız, bu işte el sıkışalım.'
                    ...
                    <div className="flex space-x-3">
                        <button className={`text-lg leading-7 font-medium ${isDarkMode ? 'bg-[#3730A3] text-white' : 'bg-[#BAB2E7] text-[#252128]'} px-8 py-3 rounded-lg`}>
